...
def evaluate(model, iterator, criterion):
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator), vector,attention,feature,inputs,paths
...
def main(args):
    ...
    # load model
    test_model.load_state_dict(torch.load("best_.txt"))
    # test acc
    criterion = nn.CrossEntropyLoss().to(device)
    test_loss, test_acc,vector,attention,feature,inputs,paths = evaluate(test_model, valid_loader, criterion)
    ...
    np.save("analysis_test/paths.npy",np.array(paths))
