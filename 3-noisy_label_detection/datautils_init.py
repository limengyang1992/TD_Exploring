...
def dataloader_finetune(batch_size):
    ...
    samples = glob.glob(f"/mnt/st_data/lmy/exps/sequence_wise/*/sequence_*.npy")
    labels = []
    for sample in samples:
        ...
        if label>0:
            if "_at_" in task: label = 4
            if "_flip_" in task: label = 1
            if "_sym_" in task: label = 1
    ...
    choose_0_train = list(choose_0.keys())[5000:40000]
    choose_0_test = list(choose_0.keys())[:5000]

    choose_1_train = list(choose_1.keys())[5000:20000]
    choose_1_test = list(choose_1.keys())[:5000]

    choose_2_train = list(choose_2.keys())[5000:20000]
    choose_2_test = list(choose_2.keys())[:5000]

    choose_3_train = list(choose_3.keys())[5000:20000]
    choose_3_test = list(choose_3.keys())[:5000]

    choose_4_train = list(choose_4.keys())[5000:20000]
...
def dataloader_finetune2(batch_size):
    ...
    train_x = choose_0_train + choose_1_train*3
    train_y = [0]*len(choose_0_train) +[1]*len(choose_1_train)*3

    test_x = choose_0_test + choose_1_test
    test_y = [0]*len(choose_0_test) +[1]*len(choose_1_test)
...
def dataloader_task(tasks,batch_size):
    
    dirs_label = {}
    for d in os.listdir("behaviour_dataset_task"):
        dirs_label[d] = np.load(f"behaviour_dataset_task/{d}/y_merge.npz")["arr_0"]

    samples = glob.glob(f"/home/kunyu/exps/sequence_wise_task/{tasks}/sequence_*.npy")
    labels = []
    
    # if "clothing" in task:
    #     target = dirs_label[f"{task}"]
    # a = target[:,:1] == target[:,1:2]
    # label = [int(1-x[0]) for x in a*1.0]
    ...
    for sample in samples:
        ...
        task = os.path.split(sample)[0].split("_d-")[1].split("__")[0]
        if "clothing" in task:
            a = dirs_label[f"{task}"][index][1]
            b = dirs_label[f"{task}"][index][0]
    ...
    test_set = [(x,y) for (x,y) in datasets.items()]
