...
class MIXUPCE(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self,pred, target_a, target_b, lam):
        left = lam.unsqueeze(-1)*F.cross_entropy(pred, target_a,reduction='none').unsqueeze(-1)
        right = (1 - lam.unsqueeze(-1)) * F.cross_entropy(pred, target_b,reduction='none').unsqueeze(-1)
