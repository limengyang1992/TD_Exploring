...
def train_one_epoch(
    ...
    for i, (index,input,target) in enumerate(train_loader):
        print(i,end=",")
        optimizer.zero_grad()
        if len(target)==4: target = torch.cat(target).view(4,-1).T
...
def validate(args, val_loader, model, criterion, epoch, writer):
    ...
    for i, batch in enumerate(val_loader):
        ...
        if len(target)==4: 
            target = torch.cat(target).view(4,-1).T
            target = target[:,:1].view(-1).long()
        target = target.cuda()
...
@torch.no_grad()
def uncertain_data_epoch(model,loader):
    model.eval()
    uncertain_epoch_1 = []
    uncertain_epoch_2 = []
    uncertain_epoch_3 = []
    ...
    total = np.concatenate([uncertain_total1, uncertain_total2], axis=1)
