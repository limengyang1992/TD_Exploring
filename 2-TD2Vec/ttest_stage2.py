...
from datautils_init import dataloader_finetune2
...
def main(args):
    ...
    train_loader,valid_loader = dataloader_finetune2(128)
    test_model = TD2VEC(class_num=2)
    ...
    np.save("pred.npy",np.array(vector))
    np.save("attn.npy",np.array(attention))
    np.save("feat.npy",np.array(feature))
